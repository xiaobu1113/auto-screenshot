...
namespace AutoScreenShotAI
{
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            string appFolder = Path.Combine(appData, "AutoScreenShotAI");
            string settingsPath = Path.Combine(appFolder, "settings.json");
            _settings = new SettingsService(settingsPath);

            _tray = new TrayService(this);
            _tray.OnRestoreRequested += RestoreFromTray;
            _tray.OnExitRequested += () => Environment.Exit(0);
        ...
        private void InitTimer()
        {
            ...
            _shotTimer.Tick += async (s, e) => await TakeScreenshotAndAnalyze();
        }
        ...
        private void LoadSettingsToUI()
        {
            ...
            if (s.IsAutoStartup) SetAutoStartup();
            if (s.IsAutoDelete) RunAutoDelete();
        ...
        private async System.Threading.Tasks.Task TakeScreenshotAndAnalyze()
        {
            try
            {
                ...
                {
                    SetStatus("Invalid output directory");
                    return;
                }

                string template = TxtFilenameTemplate.Text.Trim();
                if (string.IsNullOrWhiteSpace(template) || template.IndexOfAny(Path.GetInvalidFileNameChars()) != -1)
                {
                    SetStatus("Invalid file name template");
                    return;
                }

                int format = CombScreenshotFormat.SelectedIndex;
                string path = ScreenshotService.BuildFilePath(dir, template, format);

                using (var bmp = ScreenshotService.CaptureScreen())
                {
                    bmp.Save(path, ScreenshotService.GetImageFormat(format));
                }

                SetStatus($"Saved: {Path.GetFileName(path)}");

                if (ChkEnableScript.IsChecked != true) return;

                string script = TxtPythonPath.Text.Trim();
                if (File.Exists(script))
                {
                    SetStatus("Analyzing...");
                    string prompt = TxtPrompt.Text.Trim();
                    if (string.IsNullOrWhiteSpace(prompt)) prompt = _language.Texts.DefaultPrompt;
                    string result = await PythonService.RunScriptAsync(script, path, prompt);
        ...
        {
            bool enable = ChkAutoStartup.IsChecked == true;
            try
            {
        ...
        {
            if (ChkAutoDelete.IsChecked != true) return;
            string dir = TxtOutputDir.Text.Trim();
            if (!Directory.Exists(dir)) return;
            if (!int.TryParse(TxtAutoDeletionDays.Text, out int days) || days <= 0) return;
        ...
        private void RdoRun_Checked(object s, RoutedEventArgs e) { if (!int.TryParse(TxtInterval.Text, out int sec) || sec <= 0) { MessageBox.Show("Positive integer required"); RdoStop.IsChecked = true; return; } _shotTimer!.Interval = TimeSpan.FromSeconds(sec); _shotTimer.Start(); SetStatus("Interval started"); }
        private void RdoRun_Unchecked(object s, RoutedEventArgs e) { _shotTimer?.Stop(); SetStatus("Interval stopped"); }
        private void ChkAutoStartup_Checked(object s, RoutedEventArgs e) => SetAutoStartup();
        private void ChkAutoStartup_Unchecked(object s, RoutedEventArgs e) => SetAutoStartup();
